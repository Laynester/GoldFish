        ...
        <div class="col-sm-12">
            <div class="myPage">
                <div class="row">
                    <div class="col-sm-7">
                        @if($edit)
                            <a onclick="openStore()">{{ __('Inventory') }}</a>
                            <a onclick="openStore('store')">{{ __('Shop') }}</a>
                        @else
                            <button onclick="window.location.href='{{ route('home.edit', auth()->user()->username) }}'"
                                    class="btn">Edit
                            </button>
                        @endif
                    </div>
        ...
        <div class="row homepages">
            <div class="col-md-6">
                ...
                @foreach($stickers as $row)
                    <div id="{{$row->id}}" data-z="{{$row->z}}" data-left="{{$row->x}}" data-top="{{$row->y}}"
