...
class HomeController extends Controller
{
    public function showProfile($username)
    {
        ...
        $edit = false;
        if (\Request::is('user/home/' . $user->username . '/edit')) {
        ...
        if ($widgets->isEmpty()) {
            $data = array(
                ...
                array(
                    ...
                    'skin' => 'photo',
                    'visible' => 0,
                    'data' => '/legacy/images/empty_photo.gif'
                ),
                array(
                    'user_id' => $user->id,
                    'type' => 'n',
...
To add your buddy list to your page click edit and look in your widgets inventory. After placing it on the page you can move it all over the place and even change how it looks. Go on!'
        ...
        $friend_data = \App\Models\Hotel\Friendship::whereHas('habbo')->where('user_one_id', $user->id)->take(
            12
        )->inRandomOrder()->get();
    ...
    public function destroy()
    {
        ...
        if ($area['type'] == 'n') {
            ...
            Homes::where('user_id', auth()->user()->id)->where('id', $area['id'])->update(
    ...
    {
        $stickers = Homes::where('user_id', auth()->user()->id)->where('type', 's')->where('visible', '0')->orderBy(
            'id',
            'DESC'
        )->get();
        ...
        $bgs = Homes::where('user_id', auth()->user()->id)->where('type', 'b')->where('visible', '0')->orderBy(
            'z'
        )->get();
